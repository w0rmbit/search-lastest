...
def save_searched_domain(chat_id, domain, max_domains=20):
    ...
    if domain not in domains:
        ...
        if len(domains) > max_domains:
            domains.pop(0)  # keep only last N
...
def callback_handler(call):
    ...
    if call.data == "upload_file":
        ...
        if not domains:
            ...
            return
        markup = types.InlineKeyboardMarkup()
        for d in domains[-10:]:  # show last 10
...
# --- Use existing domain for one file ---
@bot.message_handler(func=lambda m: user_states.get(m.chat.id, "").startswith('use_existing_domain_one:'))
def handle_existing_domain_one(message):
    chat_id = message.chat.id
    state = user_states[chat_id]
    domain = state.split("use_existing_domain_one:")[1]
